...
(async () => {
    ...
    compiler.run((err, stats) => {
        if (err) {
            ...
            console.error(`\n${chalk.hex(userConfig.colors.white).bgHex(userConfig.colors.red)(" ERROR ")} Build process failed with errors.`);
        } else {
            console.log(`${chalk.hex(userConfig.colors.black).bgHex(userConfig.colors.gray)(" STATS ")}\n${stats.toString()}`);

            if (stats.hasErrors()) {
                console.error(`\n${chalk.hex(userConfig.colors.white).bgHex(userConfig.colors.red)(" ERROR ")} Build process completed with some errors.`);
            } else if (stats.hasWarnings()) {
                console.warn(`\n${chalk.hex(userConfig.colors.black).bgHex(userConfig.colors.yellow)(" WARN ")} Build process completed with some warnings.`);
            } else {
